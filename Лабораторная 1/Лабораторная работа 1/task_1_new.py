...
class PhonePhoto:
    def __init__(self, mode: str, zoom: int = 0):

        """
        Создание и подготовка к работе объекта "Фото с телефона"
        :param mode: Режим съемки; данная "модель" класса поддерживает следующие параметры: selfie, panorama, portrait, black and white
        :param zoom: Увеличение при съемке: данный параметр лежит в пределе от 0 до 10 вкулючительно, без уточнения значения при инициализации экземпляра класса - приравнивается нулю.

        Пример:
        >>> photo_1 = PhonePhoto("selfie")
        >>> print(photo_1.zoom)
        0
        """

        self.mode_list = ["selfie", "panorama", "portrait", "black and white"]

        if mode not in self.mode_list:
            raise TypeError("Неверно задан режим создания фото! Проверте, что заданный параметр относится к типу 'str' и входит в список возможных режимов, указанный в документации данного класса.")
        self.mode = mode
